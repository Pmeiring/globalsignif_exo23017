...
toy=/eos/cms/store/group/phys_susy/SOS/globalsignif_toys_negstaticv2/

get_card() {
	echo "Getting datacard and workspace from EOS"
	masspoint="$1"
	xrdcp root://eoscms.cern.ch/${eos}/cards/TChiWZ_Disp-neg_${masspoint}/card_TChiWZ_Disp-neg_${masspoint}_allEEMM.root ${TMP}/
	xrdcp root://eoscms.cern.ch/${eos}/cards/TChiWZ_Disp-neg_${masspoint}/card_TChiWZ_Disp-neg_${masspoint}_allEEMM.txt ${TMP}/
	mv ${TMP}/card_TChiWZ_Disp-neg_${masspoint}_allEEMM.root ${TMP}/mycard_${masspoint}.root
	mv ${TMP}/card_TChiWZ_Disp-neg_${masspoint}_allEEMM.txt ${TMP}/mycard_${masspoint}.txt
}

get_toys() {
	echo "Getting toys file from EOS"
	xrdcp root://eoscms.cern.ch/${toy}/toys_v2/higgsCombineTest.GenerateOnly.mH120.${SEED}.root ${TMP}/
}
...
save_toys(){
	...
	xrdcp ${TMP}/higgsCombineTest.GenerateOnly.mH120.${SEED}.root root://eoscms.cern.ch/${toy}/toys_v2/
}
